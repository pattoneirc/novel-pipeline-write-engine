...
1. 3300 字是硬红线。
   低于 3300 字，默认失败。
   除非用户明确说“这一章允许短章”，否则不能纳入正稿。

2. 3500 到 3900 字是普通章最佳区间。
   普通章默认目标应设为 3600 到 3800 字。
   这样既不容易卡红线，也不容易为了 4000 字以上而水文。

3. 3900 到 4200 字是可接受偏长区间。
   如果是实验章、打斗章、冲突章、反转章、情绪爆发章，可以保留。
   如果只是解释设定、心理独白、重复总结，需要压缩。

4. 4200 到 5000 字是特殊章节区间。
   只适合高潮章、大战章、重大失败章、大反转章、卷末章。
...
每章 3300 字为红线，3500 到 3900 字为最佳区间。
每一卷都必须按顺序写完。
当前卷未完成，禁止跳卷。
每章必须承接上一章。
每卷必须承接上一卷。
